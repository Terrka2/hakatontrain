...
from .dto import ClusterDetail, ClusterOut
        ...
        if not decisions:
            try:
                ...
                if trigger == "job_update":
                    ...
                    previous_plan = dispatch.get_plan(previous)
                ...
                # B0 orchestrates only; B6 owns the Plan store (docs/status/B6.md).
                plan = dispatch.record_plan(plan)
            except Exception:
...
def get_plan(plan_id: str) -> Plan | None:
    # B0 orchestrates only; B6 owns the Plan store (docs/status/B6.md).
    return dispatch.get_plan(plan_id)
