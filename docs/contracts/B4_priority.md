...
```
Статусы: `☐` не начато · `⏳` в работе · `✅` проверка зелёная · `❌` блокер. Все ✅ → выдай `ЗАДАЧА B4-QA` по разделу «QA перед PR» (все пункты списком, агент выполняет сам). Получил QA-отчёт со всеми ✅ или с ❌ вне путей блока → напиши «L0 готов, открывай PR» и выдай отчёт по шаблону в конце контракта. К L1 переходи только по моему сообщению `L0 принят`.
