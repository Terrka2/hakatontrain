...
```
Статусы: `☐` не начато · `⏳` в работе · `✅` проверка зелёная · `❌` блокер. Все ✅ → выдай `ЗАДАЧА L1-QA` по разделу «QA перед PR» (все пункты списком, агент выполняет сам). Получил QA-отчёт со всеми ✅ или с ❌ вне путей блока → напиши «L0 готов, открывай PR» и выдай отчёт по шаблону в конце контракта. К L1 переходи только по моему сообщению `L0 принят`.
...
Папка `docs/references/L1/` — образцы структуры и стиля кода для этого блока и общие шаблоны `docs/references/_patterns/`. Агент пишет код по ним и **не предлагает свою архитектуру, библиотеки или раскладку файлов**. Референс противоречит контракту → контракт главнее, расхождение — в отчёт.
